...
- **High/Critical Risk Positions:** 500 Sats per evaluation (Premium dynamic hedge tier).

---

## Python Integration Example

The following code shows how an agent queries `/mcp/evaluate`, handles the HTTP 402 payment challenge, pays the invoice via an LNbits node, and retrieves the signed risk evaluation:
...
def query_risk_oracle():
    ...
    try:
        ...
        if e.code == 402:
            ...
            # 3. Retry with proof of payment token
...
- View our **[Integration Cookbook & MCP Guides](./08_SDK_Wrappers/COOKBOOK.md)**.
- Discovery card: `https://api.arsenal-quant.com/.well-known/mcp/server-card.json`
