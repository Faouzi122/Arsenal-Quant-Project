...
Permet a un agent CrewAI de deleguer le calcul MEV au systeme Antigravity via L402.
"""
import os
import re
import json
...
class AntigravityRiskOracle(BaseTool):
    ...
    def _run(self, victim_weth_in: float, attacker_weth_in: float) -> str:
        ...
        if not lnbits_key:
                      return (
                                        "[ERROR] Error: LNBITS_ADMIN_KEY is not configured in the environment. "
        ...
        try:
              ...
              return f"[ERROR] Connection Error: Failed to contact the Antigravity API. {e}"

        if resp.status_code == 200:
                      return f"[OK] MEV check completed (Pre-cached/Free tier):\n{json.dumps(resp.json(), indent=2)}"

        if resp.status_code != 402:
                      return f"[ERROR] API Error: Received unexpected status code {resp.status_code}.\n{resp.text}"

        # 3. Parse L402 Challenge headers
        auth_header = resp.headers.get("WWW-Authenticate")
        if not auth_header:
                      return "[ERROR] Security Error: L402 Challenge missing WWW-Authenticate header."
        ...
        if not macaroon_match or not invoice_match:
                      return "[ERROR] Parse Error: Unable to extract Macaroon or Bolt11 invoice from challenge."
        ...
        try:
            ...
            return f"[ERROR] Wallet Error: Failed to settle Lightning invoice. {e}"
        ...
        try:
            ...
            formatted_res = (
                              f"[PROTECT] MEV RISK DETECTOR DECISION:\n"
                              f"- Signal: {signal}\n"
            ...
            return f"[ERROR] Execution Error: Paid challenge failed verification. {e}"
